        ...
        <nav className="fixed top-0 bottom-0 left-0 flex flex-col w-2/12 pt-6 pb-8 overflow-y-auto bg-gray-800">
          ...
          <div className="px-4 pb-6 space-y-4">
            ...
            <ul className="text-sm font-medium">
              {routes.map((route) => 
                  <li key={route}>
                  <a className={`flex items-center py-3 pl-3 pr-2 rounded hover:bg-gray-900 text-gray-50 ${history.location.pathname.includes(route.toLowerCase().split(' ').join('')) ? 'bg-indigo-500' : ''}`} onClick={
                    () => history.push(`/${route.toLowerCase().split(' ').join('')}`)
                  }>
                      <span>{route}</span>
                    </a>
                  </li>
            ...
            <div className="pb-8">
              ...
              <a className="flex items-center py-3 pl-3 pr-2 rounded text-gray-50 hover:bg-gray-900" href="#" onClick={
                handleLogout
              }>
                <span className="inline-block mr-4">
                </span>
