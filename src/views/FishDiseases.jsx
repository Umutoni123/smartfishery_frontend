...
import { selectFishDiseases, setFishDiseases } from '../store/modules/fishDiseasesSlice';
...
function FishDiseases() {
  ...
  const handleSubmit = () => {
    toast.promise(
      AppServices.createItem('Fishdiseases', fishDiseaseInfo),
      {
        loading: 'Creating fishDiseases ...',
        success: (response) => {
          dispatch(setFishDiseases(
            [
              ...fishDiseases,
              response.data.data
            ]
  ...
  const handleEdit = () => {
    toast.promise(
      AppServices.updateItem(`Fishdiseases/${fishDiseaseInfo.Diseases_Id}`, fishDiseaseInfo),
      {
        loading: 'Editing fishDisease ...',
        success: (response) => {
          dispatch(setFishDiseases(
            [
              ...(fishDiseases.filter((el) => el.Diseases_Id !== fishDiseaseInfo.Diseases_Id)),
              response.data.data
            ]
          ));
          closeModal();
  ...
  const handleDelete = () => {
    toast.promise(
      AppServices.deleteItem(`Fishdiseases/${fishDiseaseInfo.Diseases_Id}`),
      {
        loading: 'Deleting fishDisease ...',
        success: (response) => {
          dispatch(setFishDiseases(
            [
              ...(fishDiseases.filter((fishDiseases) => fishDiseases.Diseases_Id !== fishDiseaseInfo.Diseases_Id)),
            ]
          ));
          closeModal();
                    ...
                    (e) => {
                      ...
                      <label className="block mb-4">
                        ...
                        <input
                          ...
                          id="signInInput1-1"
                          onChange={
                            (e) => setFishDiseaseInfo({ ...fishDiseaseInfo, Disease_name: e.target.value })
                          }
                          type="text"
                      ...
                      {/* <label className="block mb-5">
                        ...
                        <select required className="px-4 py-3.5 w-full text-gray-400 font-medium placeholder-gray-400 bg-white outline-none border border-gray-300 rounded-lg focus:ring focus:ring-indigo-300" onChange={
                          (e) => setFishDiseaseInfo({ ...fishDiseaseInfo, coopid: e.target.value })
                        }>
                          <option value=""></option>
                  ...
                  {fishDiseases.map((fishDisease) => (
                    <tr>
                      ...
                      <td className="px-6 py-4 text-sm font-medium text-gray-800 whitespace-nowrap">
                        {fishDisease.Diseases_Id}
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-800 whitespace-nowrap">
                        {fishDisease.Disease_name}
                      </td>
                      {/* <td className="px-6 py-4 text-sm text-gray-800 whitespace-nowrap">
                        {fishDisease.coopid}
                      </td> */}
